...
function getNextFriday(date) {
  ...
  const diff = dayOfWeek < 3 ? friday - dayOfWeek : friday - dayOfWeek + 7;
...
function getCountWeekendsInMonth(month, year) {
  ...
  let weekendCount = 0;
  while (length) {
    const dayOfWeek = new Date(year, month - 1, length).getDay();
...
function getNextFridayThe13th(date) {
  const FRIDAY = 5;
  const SEARCHEDDATE = 13;
  const year = date.getFullYear();
  let currentMonth = date.getMonth();
  let currentDate = new Date(year, currentMonth, SEARCHEDDATE);
  let length = 199;

  while (length) {
    currentDate = new Date(year, currentMonth, SEARCHEDDATE);
    const isFriday = currentDate.getDay() === FRIDAY;
    if (isFriday) {
      return currentDate;
    }
    currentMonth += 1;
